...
if ( ! function_exists( 'designfly_entry_footer' ) ) :
	...
	function designfly_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			/* translators: used between list items, there is a space after the comma */
			$tagss = get_the_tags(get_the_ID());
			if($tagss)
			{	
				echo "TAGS : ";
				foreach($tagss as $tags)
				{
					echo '| <a href="#" class="red_link">'.$tags->name.'</a> ';
				}
				echo "|";
			}
		}
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text" style="width:100%;float:left;">%s</span>', 'designfly' ),
					array(
						'span' => array(
							'class' => array(),
						),
			...
			'<span class="edit-link" style="width:100%;float:left;">',
			'</span>'
		);
	}
endif;
